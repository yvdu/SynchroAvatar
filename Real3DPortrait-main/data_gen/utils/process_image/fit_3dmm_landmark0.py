...
import argparse
import tqdm
from utils2.commons.multiprocess_utils import multiprocess_run_tqdm
...
def fit_3dmm_for_a_image(img_name, debug=False, keypoint_mode='mediapipe', device="cuda:0", save=True):
    ...
    t2 = time.time()
    print(t2 - t1,"extract_lm478_from_img_name")
    ...
    t2 = time.time()
    print(t2 - t1, "optimizer")

    t1 = time.time()

    # 其他参数初始化，先训练euler和trans 200 18.3
    for _ in range(150):
    ...
    for i in range(int((num_frames-1)/batch_size+1)):
        ...
        for j in range(50):
            ...
            print(loss, "loss50",j)
        face_shape = face_model.compute_shape(id_para, exp_para)
        face_norm = face_model.compute_norm(face_shape)
        normal_map = face_norm.detach().cpu().numpy()
        ...
        # 获取顶点坐标
        vertices = face_model.compute_shape(id_para, exp_para).detach().cpu().numpy()[0]  # 取第一个样本

        # 获取面索引
        faces = face_model.face_buf.detach().cpu().numpy()
    ...
    t2 = time.time()
    print(t2 - t1, "fine fitting the 3DMM in batches")
