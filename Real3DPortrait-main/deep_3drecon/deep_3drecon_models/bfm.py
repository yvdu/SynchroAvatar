...
@torch.jit.script
def compute_rotation_jit(angles: torch.Tensor):
    batch_size = angles.shape[0]
    ones = torch.ones([batch_size, 1], device=angles.device)
    zeros = torch.zeros([batch_size, 1], device=angles.device)
    ...
    rot_y = torch.cat([torch.cos(y), zeros, torch.sin(y),
                       zeros, ones, zeros,
                       -torch.sin(y), zeros, torch.cos(y)], dim=1).reshape([batch_size, 3, 3])

    rot_z = torch.cat([torch.cos(z), -torch.sin(z), zeros,
...
class ParametricFaceModel:
    def __init__(self, 
        ...
        #保存mean_tex：基础颜色贴图（Base Color Texture）
        #tex_base：纹理变形基底（类似 BlendShape 对应的纹理调整）

        base_color_texture = self.mean_tex.reshape(-1, 3)  # 还原到 RGB 格式
        base_color_texture = (base_color_texture / 255.).astype(np.float32)  # 归一化到 [0,1]

        np.save("base_color_texture.npy", base_color_texture)  # 保存为 NumPy 数组
        cv2.imwrite("base_color_texture.png", (base_color_texture * 255).astype(np.uint8))
        # face indices for each vertex that lies in. starts from 0. [N,8]
        self.point_buf = model['point_buf'].astype(np.int64) - 1
        # vertex indices for each face. starts from 0. [F,3]
        self.face_buf = model['tri'].astype(np.int64) - 1
        #np.save('3dmm_faces.npy', self.face_buf)
        #print(self.face_buf,"self.face_buf",self.face_buf.shape)
        ...
        #np.save("3dmm_mean_shape.npy", self.mean_shape)  # 3DMM 基础形状
        #np.save("3dmm_id_base.npy", self.id_base)  # 形状变形基底
        #np.save("3dmm_exp_base.npy", self.exp_base)  # 表情变形基底

        #UV坐标
        #vertices = self.mean_shape.reshape(-1, 3)  # 获取 3D 顶点坐标
        #faces = self.face_buf  # 获取三角形面片索引

        # 计算 UV 贴图（简单展开到 2D）
        #uv_map = vertices[:, :2]  # 直接取前两个坐标作为 UV
        #uv_map = (uv_map - uv_map.min(axis=0)) / (uv_map.max(axis=0) - uv_map.min(axis=0))  # 归一化

        #np.save('3dmm_uv.npy', uv_map)
    ...
    def compute_key_shape(self, id_coeff, exp_coeff):
        ...
        #BlendShape 权重
        blendshape_basis = self.exp_base.reshape(-1, 64)  # 形变基底
        blendshape_weights = np.dot(exp_coeff.detach().cpu().numpy(), blendshape_basis.detach().cpu().numpy().T)  # 计算权重

        np.save("blendshape_basis.npy", blendshape_basis.cpu().numpy())  # BlendShape 基底
        np.save("blendshape_weights.npy", blendshape_weights)  # BlendShape 权重
    ...
    def compute_color(self, face_texture, face_norm, gamma):
        ...
        print(face_color,"face_color",face_color.shape)
        return face_color
    ...
    def to_camera(self, face_shape):
        return torch.cat([face_shape[..., :-1], self.camera_distance - face_shape[..., -1:]], dim=-1)
    ...
    def compute_for_landmark_fit(self, id, exp, angles, trans, ret=None):
        ...
        rotation = compute_rotation_jit(angles)
