...
print(project_root)
sys.path.append(project_root)  # 添加项目根目录到 Python 路径
for p in sys.path:
    print(p)
...
from data_gen.utils.process_image.fit_3dmm_landmark0 import fit_3dmm_for_a_image
...
# common utils
from utils2.commons.hparams import hparams, set_hparams
from utils2.commons.tensor_utils import move_to_cuda, convert_to_tensor
from utils2.commons.ckpt_utils import load_ckpt, get_last_checkpoint
...
def cal_face_area_percent(img_name):
    print(img_name,"img_nameimg_name")
...
def crop_img_on_face_area_percent(img_name, out_name='temp/cropped_src_img.png', min_face_area_percent=0.2):
    ...
    print("crop_img_on_face_area_percent",min_face_area_percent)
    try:
    ...
    if face_area_percent >= min_face_area_percent:
        ...
        return out_name,(int(new_min_x * size_x),int(new_max_x * size_x),int(new_min_y * size_y),int(new_max_y * size_y))
...
class GeneFace2Infer:
    def __init__(self, audio2secc_dir, head_model_dir, torso_model_dir, device=None, inp=None):
        ...
        print(head_model_dir,"head_model_dir")
        print(torso_model_dir,"torso_model_dir")
        self.device = device
    ...
    def infer_once(self, inp):
        ...
        self.inp = inp
        t1 = time.time()
        samples = self.prepare_batch_from_inp(inp)#26.18718409538269
        t2 = time.time()
        seed = inp['seed'] if inp['seed'] is not None else int(time.time())
        random.seed(seed)
        torch.manual_seed(seed)
        np.random.seed(seed)
        out_name = self.forward_system(samples, inp)#3.0618700981140137
        t3 = time.time()
        print(t2 - t1, "prepare_batch_from_inp")
        print(t3 - t2, "forward_system")
        #{'drv_audio_name': 'silent_audio_short.wav', 'src_image_name': 'infer_out/tmp/cropped_src_img.png', 'a2m_ckpt': 'checkpoints/240210_real3dportrait_orig/audio2secc_vae', 'head_ckpt': '', 'torso_ckpt': 'checkpoints/240210_real3dportrait_orig/secc2plane_torso_orig', 'bg_image_name': '', 'drv_pose_name': 'data/raw/examples/May_5s.mp4', 'blink_mode': 'period', 'temperature': 0.2, 'mouth_amp': 0.45, 'out_name': '1', 'out_mode': 'final', 'map_to_init_pose': 'True', 'head_torso_threshold': None, 'seed': None, 'min_face_area_percent': 0.2, 'low_memory_usage': False}
        return out_name
    ...
    def prepare_batch_from_inp(self, inp):
        ...
        print(t0001 - t0000, "Process Driving Motion")
        t0000 = time.time()
        # Face Parsing 2.01432204246521
        ...
        sample['segmap'] = torch.tensor(segmap).float().unsqueeze(0).cuda()#(6, 512, 512)
        ...
        print(t0001 - t0000, "Face Parsing0000000000")
        #t0000 = time.time()

        # 3DMM, get identity code and camera pose
        coeff_dict = fit_3dmm_for_a_image(image_name, save=False) #23.21072793006897

        #0.0015
        t0000 = time.time()

        print(t0000 - t0001, "Face Parsing0000000000")
        ...
        #print(sample['src_kp'],"sample['src_kp']",sample['src_kp'].shape)
        ...
        print(t0001 - t0000, "Face Parsing")

        t0000 = time.time()
        print(inp['drv_pose_name'],"inp['drv_pose_name']")
        # extract camera pose 63.30288863182068
        if inp['drv_pose_name'] == 'static':
            ...
            if inp['drv_pose_name'].endswith('.mp4'):
                ...
                drv_pose_coeff_dict = fit_3dmm_for_a_video(inp['drv_pose_name'], save=True)
            else:
                # load from npy
        ...
        print(t0001 - t0000, "prepare camera")
        # fix the z axis
        ...
        # prepare camera 0.0024538040161132812
    ...
    def get_mfcc(self, wav16k_name):
        from utils2.audio import librosa_wav2mfcc
    ...
    def forward_secc2video(self, batch, inp=None):
            ...
            with torch.no_grad():
                for i in tqdm.trange(num_frames, desc="Real3D-Portrait is rendering frames"):
                    ...
                    if i == 0:
                        ...
                        img = ((gen_output['image'] + 1) / 2 * 255.).permute(0, 2, 3, 1)[0].int().cpu().numpy().astype(
                            np.uint8)
                        cv2.imwrite('./infer_out/end1.png', img)
                    else:
            ...
            with torch.no_grad():
                for i in tqdm.trange(num_frames, desc="Real3D-Portrait is rendering frames"):
                    ...
                    if i == 0:
                        ...
                        gen_output_img = self.secc2video_model.forward(img=ref_img_head, camera=camera[i:i+1], cond=cond, ret={}, cache_backbone=True, use_cached_backbone=False)
                        img_0.append(gen_output_img['image'])
                        img_0 = torch.cat(img_0)
                        img_0 = img_0.cpu()
                        img_0 = img_0.clamp(-1, 1)

                        out_0 = ((img_0.permute(0, 2, 3, 1) + 1) / 2 * 255).int().cpu().numpy().astype(np.uint8)
                        out_0 = out_0[0]
                        t = log_current_time(log_file)
                        t001 = time.time()
                        #1.594
                        print(t001 - t000,"forward_time")
                        dir = inp['out_name']
                        inp['out_name'] = t
                        inp['out_name'] = f'../picture/{dir}/' + inp['out_name'] +'_1'+ '.png'
                        out_fname_0 = 'infer_out/tmp/' + os.path.basename(inp['src_image_name'])[
                                                         :-4] + '_' + os.path.basename(inp['drv_pose_name'])[
                                                                      :-4] + '.png' if inp['out_name'] == '' else inp[
                            'out_name']
                        try:
                            os.makedirs(os.path.dirname(out_fname_0), exist_ok=True)
                        except:
                            pass

                        print(inp['out_name'] )
                        print(inp['out_name'],"inp['out_name']")
                        out_0[..., [0, 2]] = out_0[..., [2, 0]]
                        img_a = cv2.imread(inp['src_image_name0'])

                        img_b = out_0
                        new_min_x, new_max_x, new_min_y, new_max_y = batch["area"]
                        target_width = new_max_x - new_min_x
                        target_height = new_max_y - new_min_y
                        img_b_resized = cv2.resize(img_b, (target_width, target_height))
                        print(img_b.shape,"img_b")
                        print(target_width, target_height,"resize")

                        print(batch["area"],"src")
                        print(inp['src_image_name0'], "src0000000000000000")
                        cv2.imwrite(inp['out_name'], img_b_resized)
                        gen_output = self.secc2video_model.forward(img=ref_img_head, camera=camera[i:i+1], cond=cond, ret={}, cache_backbone=True, use_cached_backbone=False)
        ...
        out_fname = 'infer_out/tmp/' + os.path.basename(inp['src_image_name'])[:-4] + '_' + os.path.basename(inp['drv_pose_name'])[:-4] + '.mp4'
        try:
    ...
    def forward_system(self, batch, inp):
        ...
        t01 = time.time()
        #0.743
        print(t01 - t00, "forward_audio2secc")
        t = log_current_time(log_file)
        t00 = time.time()
        out_fname = self.forward_secc2video(batch, inp) #2.4081220626831055
        t01 = time.time()
        print(t01 - t00, "forward_secc2video")
        return out_fname
    #初始化，省不了
    @classmethod
    def example_run(cls, inp=None):
        print(inp['drv_audio_name'], "inp['drv_audio_name']00")
        ta = time.time()
        inp_tmp = {
        ...
        if inp is not None:
            inp_tmp.update(inp)
        inp = inp_tmp
        print(inp['drv_audio_name'],"inp['drv_audio_name']")
        infer_instance = cls(inp['a2m_ckpt'], inp['head_ckpt'], inp['torso_ckpt'], inp=inp)
        tb = time.time()
        print(tb - ta,"infer_instance") #8.954088687896729
    ...
    def render_static_3dmm(self, img_path, output_path="static_3dmm.png"):
        """ 渲染静态 3DMM 投影图 """
        tmp_img_name = 'infer_out/tmp/cropped_src_img.png'
        a, area = crop_img_on_face_area_percent(img_path, tmp_img_name,
                                                min_face_area_percent=0.2)


        coeff_dict = fit_3dmm_for_a_image(tmp_img_name, save=False)
        assert coeff_dict is not None, "3DMM 参数提取失败！"

        # 提取 3DMM 参数
        src_id = torch.tensor(coeff_dict['id']).reshape([1, 80]).cuda()
        src_exp = torch.zeros([1, 64]).cuda()  # 使表情归零
        src_euler = torch.tensor([0, 0, 0]).reshape([1, 3]).cuda()  # 使角度归零
        src_trans = torch.tensor([0, 0, 0]).reshape([1, 3]).cuda()

        # 渲染 3DMM 模型
        _, rendered_img = self.secc_renderer(src_id, src_exp, src_euler, src_trans)

        # 转换格式并保存
        rendered_img = ((rendered_img + 1) / 2 * 255).permute(0, 2, 3, 1)[0].cpu().numpy().astype(np.uint8)
        cv2.imwrite(output_path, rendered_img)
        print(f"已保存静态 3DMM 正面投影图：{output_path}")

        return output_path

    def render_static_front_view(self, img_path, output_path="static_portrait.png"):
        """基于输入图片，合成3D模型，并渲染出静态正面头像"""
        print(f"处理图片: {img_path}")
        tmp_img_name = 'infer_out/tmp/cropped_src_img.png'
        a, area = crop_img_on_face_area_percent(img_path, tmp_img_name,
                                                min_face_area_percent=0.2)

        # 提取3DMM参数（基于输入图片生成3D模型）
        coeff_dict = fit_3dmm_for_a_image(tmp_img_name, save=False)
        assert coeff_dict is not None, "3DMM 参数提取失败！"

        # 转换为 Tensor 并设定静态参数
        src_id = torch.tensor(coeff_dict['id']).reshape([1, 80]).cuda()
        src_exp = torch.zeros([1, 64]).cuda()  # 归零表情，使其静态
        src_euler = torch.tensor([0, 0, 0]).reshape([1, 3]).cuda()  # 归零角度
        src_trans = torch.tensor([0, 0, 5]).reshape([1, 3]).cuda()  # 使 Z 轴位置固定，确保正面

        # 使用 SECC_Renderer 进行渲染
        _, rendered_img = self.secc_renderer(src_id, src_exp, src_euler, src_trans)

        # 格式转换并保存
        rendered_img = ((rendered_img + 1) / 2 * 255).permute(0, 2, 3, 1)[0].cpu().numpy().astype(np.uint8)
        cv2.imwrite(output_path, rendered_img)
        print(f"静态正面渲染完成，已保存到: {output_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--drv_pose", default='data/raw/examples/May_5s_coeff_fit_mp.npy', type=str) # data/raw/examples/May_5s.mp4
    ...
    parser.add_argument("--pic_name", default='')
