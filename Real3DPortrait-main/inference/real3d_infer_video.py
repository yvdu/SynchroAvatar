...
class GeneFace2Infer:
    ...
    def prepare_bg_image(self, bg_path, crop_position="center"):
        """
        读取背景图，并根据 crop_position 裁剪或调整大小。
        crop_position 可以是：
            "left_top"、"right_top"、"left_bottom"、"right_bottom" 或 "center"
        """
        ...
        # 如果图片太小，直接 resize 到 512x512
        if h < target_size or w < target_size:
            bg_img = cv2.resize(bg_img, (target_size, target_size))
            return bg_img

        # 根据位置裁剪 512x512 区域
        if crop_position == "left_top":
            x1, y1 = 0, 0
    ...
    def forward_secc2video(self, batch, inp=None):
        ...
        if inp['low_memory_usage']:
            ...
            with torch.no_grad():
                for i in tqdm.trange(num_frames, desc="Real3D-Portrait is rendering frames"):
                    kp_src = torch.cat([src_kps[i:i + 1].reshape([1, 68, 2]),
                                        torch.zeros([1, 68, 1], device=src_kps.device)], dim=-1)
                    kp_drv = torch.cat([drv_kps[i:i + 1].reshape([1, 68, 2]),
                                        torch.zeros([1, 68, 1], device=drv_kps.device)], dim=-1)
                    cond = {
                    ...
                    if i == 0:
                        gen_output_img = self.secc2video_model.forward(
                            img=ref_img_head, camera=camera[i:i + 1], cond=cond,
                            ret={}, cache_backbone=True, use_cached_backbone=False
                        )
                        img_0.append(gen_output_img['image'])
                        img_0 = torch.cat(img_0).cpu().clamp(-1, 1)

                        out_0 = ((img_0.permute(0, 2, 3, 1) + 1) / 2 * 255).int().numpy().astype(np.uint8)
                        out_0 = out_0[0]
                        ...
                        out_fname_0 = 'infer_out/tmp/' + os.path.basename(inp['src_image_name'])[:-4] \
                                      + '_' + os.path.basename(inp['drv_pose_name'])[:-4] + '.png' \
                            if inp['out_name'] == '' else inp['out_name']
                        try:
                        ...
                        gen_output = self.secc2video_model.forward(
                            img=ref_img_head, camera=camera[i:i + 1], cond=cond,
                            ret={}, cache_backbone=False, use_cached_backbone=True
                        )

                    img_lst.append(gen_output['image'].cpu())  # 立刻搬到 CPU
                    img_raw_lst.append(gen_output['image_raw'].cpu())
                    depth_img_lst.append(gen_output['image_depth'].cpu())
            # save demo video
            depth_imgs = torch.cat(depth_img_lst, dim=0)
            imgs = torch.cat(img_lst, dim=0)
            imgs_raw = torch.cat(img_raw_lst, dim=0)

            # ✅ 改成逐帧 interpolate + 搬 CPU，避免一次性占满显存
            secc_img_lst = []
            for i in range(num_frames):
                img = torch.nn.functional.interpolate(drv_secc_colors[i:i + 1].cpu(), (512, 512))
                secc_img_lst.append(img)
            secc_img = torch.cat(secc_img_lst, dim=0)

            if inp['out_mode'] == 'concat_debug':
                secc_img = ((secc_img + 1) * 127.5).permute(0, 2, 3, 1).int().numpy()

                depth_img = F.interpolate(depth_imgs, (512, 512))
                ...
                imgs = torch.cat([
                    ref_img_gt.repeat([imgs.shape[0], 1, 1, 1]),
                    secc_img,
                    F.interpolate(imgs_raw, (512, 512)),
                    depth_img,
                    imgs
                ], dim=-1)
