...
def crop_img_on_face_area_percent(img_name, out_name='temp/cropped_src_img.png', min_face_area_percent=0.05):
    try:
...
class GeneFace2Infer:
    ...
    def prepare_batch_from_inp(self, inp):
        ...
        #print(src_id,"src_id",src_id.shape)torch.Size([1, 80])
        #print(src_exp, "src_exp",src_exp.shape)torch.Size([1, 64])
        #print(src_euler, "src_euler",src_euler.shape)torch.Size([1, 3])
        #print(src_trans, "src_trans",src_trans.shape)torch.Size([1, 3])
    ...
    def forward_audio2secc(self, batch, inp=None):
        ...
        '''hubert torch.Size([1, 8, 1024])
        f0 torch.Size([1, 8])
        x_mask torch.Size([1, 8])
        y_mask torch.Size([1, 4])
        blink torch.Size([1, 8, 1])
        audio torch.Size([1, 8, 1024])
        eye_amp torch.Size([1, 1])
        mouth_amp torch.Size([1, 1])
        ref_gt_img torch.Size([1, 3, 512, 512])
        segmap torch.Size([1, 6, 512, 512])
        ref_head_img torch.Size([1, 3, 512, 512])
        ref_torso_img torch.Size([1, 3, 512, 512])
        bg_img torch.Size([1, 3, 512, 512])
        id torch.Size([4, 80])
        src_kp torch.Size([4, 68, 2])
        euler torch.Size([4, 3])
        trans torch.Size([4, 3])
        camera torch.Size([4, 25])
        exp torch.Size([4, 64])'''
            ...
            with torch.no_grad():
                for i in tqdm.trange(num_frames, desc="Real3D-Portrait is rendering frames"):
                    ...
                    if i == 0:
                        ...
                        dir = inp['out_name']
                        inp['out_name'] = t
                        inp['out_name'] = f'../picture/{dir}/' + inp['out_name'] + '.png'
                        ...
                        out_0[..., [0, 2]] = out_0[..., [2, 0]]
                        cv2.imwrite(inp['out_name'], out_0)
                        gen_output = self.secc2video_model.forward(img=ref_img_head, camera=camera[i:i+1], cond=cond, ret={}, cache_backbone=True, use_cached_backbone=False)
